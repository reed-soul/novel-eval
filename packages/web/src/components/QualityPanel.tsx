...
export function QualityPanel({ projectId }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([s, l]) => { setScores(s.scores); setLessons(l.lessons); })
      .catch(() => {})
  ...
  if (scores.length === 0 && lessons.length === 0) {
    return <div style={{ color: 'var(--muted)', fontSize: 14 }}>暂无评估数据（写完章节后自动生成）</div>;
  }
      ...
      {tab === 'trend' && (
        <div>
          <div style={{ display: 'flex', gap: 24, marginBottom: 12, fontSize: 14 }}>
            <span>平均分：<strong style={{ color: GRADE_COLOR[avgScore >= 80 ? 'A' : avgScore >= 75 ? 'B' : 'C'] }}>{avgScore}</strong></span>
            <span>A 级：<strong style={{ color: '#16a34a' }}>{aCount}</strong> / {scores.length}</span>
          </div>
          {/* 简易柱状图：每章一个竖条，高度按分数 */}
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 100, overflowX: 'auto', padding: '4px 0' }}>
            {scores.map((s) => (
              <div
                key={s.chapter}
                title={`第 ${s.chapter} 章：${s.score} 分（${s.grade}）`}
                style={{
                  minWidth: 8,
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: 'var(--muted)' }}>
            颜色：<span style={{ color: GRADE_COLOR.A }}>■A(80+)</span> <span style={{ color: GRADE_COLOR.B }}>■B(70-79)</span> <span style={{ color: GRADE_COLOR.C }}>■C(60-69)</span>
          </div>
        </div>
      )}
